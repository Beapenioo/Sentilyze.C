...
NEGATIVE_WORDS_TR = {
    ...
    "iade": -1.0, "boşa": -1.0, "düşük kalite": -1.2
}
...
def analyze_bert_tr(text: str) -> Tuple[str, float]:
...
async def analyze(req: AnalyzeRequest):
    try:
        ...
        bert_label, bert_score = analyze_bert_tr(text)
        
        # Kural tabanlı analiz
        rule_label, rule_score = analyze_rule_based_tr(text)
        
        # Ağırlıklı ortalama hesapla (BERT: 0.7, Rule-based: 0.3)
        ...
        explanation = f"BERT: {bert_label} ({bert_score:.2f}), Rule-based: {rule_label} ({rule_score:.2f})"
